...
with torch.no_grad():
    for t in reversed(range(T)):
        ...
        # 预测噪声
        predicted_noise = diffusion_model(xt, torch.tensor(t).float().to(device))


        # 反向扩散
        xt = backward_diffusion(xt, t, predicted_noise, alphas,sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod,beta_schedule,posterior_variance)
        # 保存当前步的图像
        save_image(xt, f"test_results/{t}xt.png", normalize=False)
        # 打印当前步的值范围和 alpha 信息
