...
from diffusion_transformer_train_learning_sigma import forward_diffusion, beta_schedule, alphas, posterior_variance, transform, \
    image_size, model_path
import matplotlib.pyplot as plt
from diffusion_transformer_train_learning_sigma import backward_diffusion, generate_random_square_mask, sqrt_alphas_cumprod, \
...
with torch.no_grad():
    for t in reversed(range(T)):
        ...
        # 预测噪声
        output = diffusion_model(xt * (1 - mask) + imgs * mask, torch.tensor([t]).to(device),torch.tensor([0]).to(device))
        ...
        print(f"Step {t}: log_sigma_squared min {log_sigma_squared.min().item()}, max {log_sigma_squared.max().item()}, mean {log_sigma_squared.mean().item()}")
